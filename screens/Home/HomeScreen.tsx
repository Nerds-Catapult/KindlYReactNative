  ...
  return (
    <View className='w-full h-full'>
      <View className="h-20 w-full flex flex-row justify-between items-center p-6">
        ...
        <Text className='text-[#1C122D] text-serif text-2xl font-bold'>Kndly</Text>
      ...
      <View className='w-full h-40 flex flex-col items-start p-4 space-y-4 mt-10'>
        <Text className='text-2xl font-bold font-serif tracking-wider'>Cycle through our library</Text>
        <View className='w-full flex flex-row items-center justify-between space-x-2'>
          <TextInput className='p-2 rounded bg-gray-200 h-12 w-[80%] text-purple-900' placeholder="Search" />
      ...
      <View className='flex flex-col items-start w-full h-[70%] bg-[#E4E4E4] p-4 rounded-t-xl'>
        <View className='h-14 w-full flex flex-row px-4 items-center justify-between'>
          ...
          <Text className='text-sm font-light text-purple-900 underline'> View All &rarr;</Text>
        </View>
        <ScrollView horizontal>
          <View className='flex flex-row space-x-2 h-12 items-center'>
            {
              categories.map((category, index) => (
                <Chip className='bg-gray-100' rippleColor={"#CB37FF"} showSelectedCheck selectedColor='#7737FF' icon="more">{category.name}</Chip>
              ))
            }
          </View>
        </ScrollView>
        ...
        <View className='h-10 w-full flex flex-row justify-between items-center px-4'>
          ...
          <Text className='font-bold text-purple-800 text-sm underline'>View More &rarr;</Text>
        </View>
        <FlatList
          horizontal
          data={books}
